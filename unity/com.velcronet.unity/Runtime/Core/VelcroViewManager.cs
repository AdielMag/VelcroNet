...
{
    /// <summary>
    /// The bridge between the VelcroPhysics simulation and the Unity scene.
    /// One instance per scene — attach to a persistent manager GameObject.
    ///
    /// Responsibilities:
    ///   - Creates and ticks PhysicsWorldManager at a fixed rate.
    ///   - Registers all VelcroRigidbody components and their colliders.
    ...
    /// </summary>
    [AddComponentMenu("VelcroNet/View Manager")]
    [DisallowMultipleComponent]
    public sealed class VelcroViewManager : MonoBehaviour, IFullCollisionSink
    {
        // ─── Inspector ───────────────────────────────────────────────────────
        ...
        private VelcroRigidbody[]           _bodies;
        private IVelcroCollisionHandler?[]  _collisionHandlers;
        private IVelcroTriggerHandler?[]    _triggerHandlers;
        ...
        // ─── Singleton access ─────────────────────────────────────────────────
        public static VelcroViewManager? Instance { get; private set; }
        ...
        private void Awake()
        {
            ...
            {
                Debug.LogWarning("[VelcroNet] Multiple VelcroViewManager instances detected. Destroying duplicate.");
                Destroy(gameObject);
                return;
            }
            Instance = this;
            ...
            _bodies            = new VelcroRigidbody[_maxBodies];
            _collisionHandlers = new IVelcroCollisionHandler?[_maxBodies];
            _triggerHandlers   = new IVelcroTriggerHandler?[_maxBodies];
            ...
            VelcroPhysicsQueries.Initialize(_physicsWorld, _queryBuffer);
        ...
        private void RegisterSceneBodies()
        {
            ...
            var rigidbodies = FindObjectsByType<VelcroRigidbody>(FindObjectsSortMode.None);
#else
            var rigidbodies = FindObjectsOfType<VelcroRigidbody>();
#endif
        ...
        private void RegisterBody(VelcroRigidbody rb, int entityId)
        {
            if (_bodyCount >= _maxBodies)
            {
                Debug.LogError($"[VelcroNet] MaxBodies ({_maxBodies}) exceeded. Entity '{rb.name}' not registered.");
                return;
            }
            ...
            var colliders = rb.GetComponents<IVelcroColliderProvider>();
            ...
            _bodies[entityId]            = rb;
            _collisionHandlers[entityId] = rb.GetComponent<IVelcroCollisionHandler>();
            _triggerHandlers[entityId]   = rb.GetComponent<IVelcroTriggerHandler>();
            _bodyCount++;
        }
        ...
        /// </summary>
        public int RegisterDynamic(VelcroRigidbody rb)
        {
            // Find a free slot
            int entityId = -1;
            for (int i = 0; i < _maxBodies; i++)
